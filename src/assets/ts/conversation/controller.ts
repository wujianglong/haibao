...
conversationCtr.controller("conversationController", ["$scope", "$state", "mainDataServer", "conversationServer", "mainServer", "RongIMSDKServer", "$http", "$timeout", "$location", "$anchorScroll",
  function ($scope: any, $state: angular.ui.IStateService, mainDataServer: mainDataServer, conversationServer: conversationServer, mainServer: mainServer, RongIMSDKServer: RongIMSDKServer, $http: angular.IHttpService, $timeout: angular.ITimeoutService, $location: angular.ILocationService, $anchorScroll: angular.IAnchorScrollService) {
    ...
    var messageItem: any = {
      ...
      1: function (image: any) {
        return new RongIMLib.ImageMessage({ content: image.thumbnail, imageUri: image.downloadUrl });
      }
    };
    var createMessage = function (file: any) {
      var msg = messageItem[file.fileType](file);
      return msg;
    };

    var urlItem: any = {
      4: function (data: any, callback: any) {
        RongIMSDKServer.getFileUrl(4, data.filename, data.name).then(function (result: any) {
      ...
      1: function (data: any, callback: any) {
        RongIMSDKServer.getFileUrl(1, data.filename, null).then(function (result: any) {
          data.downloadUrl = result.downloadUrl;
          var message = createMessage(data);
          callback(message);
        }, function (error: any) {
    ...
    var getFileUrl = function (data: any, callback: any) {
      urlItem[data.fileType](data, callback);
    };

    var callbacks = {
      onError: function (error: any, type: number, messageId?: string) {
        uploadError(type, messageId, error);
      },
      onProgress: function (loaded: number, total: number, type: number, messageId?: string) {
    ...
    // 发送文件消息
    var fileNode: any = document.getElementById('upload-file');
    fileNode.onchange = function () {
      ...
      var config = {
        domain: UPLOADDOMAIN,
        fileType: 4,
        getToken: function (callback: any) {
          RongIMSDKServer.getFileToken(this.fileType).then(function (data: any) {
            callback(data.token);
          }, function (error: any) {
      ...
      UploadClient.initFile(config, function (uploadFile: any) {
        fileNode.value = '';
        ...
        uploadFile.upload(file, {
          ...
          onProgress: function (loaded: number, total: number) {
    ...
    // 发送图片消息
    var imageNode: any = document.getElementById('upload-image');
    imageNode.onchange = function () {
      ...
      UploadClient.initImage(config, function (uploadFile: any) {
        imageNode.value = '';
    ...
    function uploadError(type: number, messageId: string, err: any) {
      if (type == 1) {
        $scope.uploadStatus.show = false;
    ...
    function uploadBase64(strBase64: string, file: any) {
      ...
      UploadClient.initImgBase64(imgConfig, function (uploadFile: any) {
        uploadFile.upload(strBase64, {
          onError: function (error: any) {
            callbacks.onError(error, imgConfig.fileType);
          },
          onProgress: function (loaded: any, total: any) {
            callbacks.onProgress(loaded, total, imgConfig.fileType)
          },
          onCompleted: function (result: any) {
            showLoading(false);
            $scope.showPasteDiv(false);
